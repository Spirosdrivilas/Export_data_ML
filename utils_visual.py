...
def plot_heatmap(df: pd.DataFrame):
    """Visualizes the correlation heatmap of the DataFrame's numeric values."""
    plt.figure(figsize=(14, 10))
    
    # Κρατάμε μόνο τις αριθμητικές στήλες (αγνοούμε το 'Label')
    numeric_df = df.select_dtypes(include=[np.number])
    
    # Υπολογίζουμε τη συσχέτιση (correlation)
    corr_matrix = numeric_df.corr()
    
    # Σχεδιάζουμε το heatmap της συσχέτισης
    sns.heatmap(corr_matrix, cmap='plasma', annot=False)
